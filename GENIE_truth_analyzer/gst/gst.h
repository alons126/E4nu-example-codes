...
gst::gst(TTree* tree, const std::string input_files) : fChain(0) {
    // if parameter tree is not specified (or zero), connect the file
    // used to generate this class and read the Tree.
    if (tree == 0) {
        TFile* f = (TFile*)gROOT->GetListOfFiles()->FindObject(input_files.c_str());

        if (!f || !f->IsOpen()) { f = new TFile(input_files.c_str()); }
